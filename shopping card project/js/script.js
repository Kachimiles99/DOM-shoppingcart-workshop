//importing the element using the DOM
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  for (let i = 0; i < addbuttons.length; i++) {
    addbuttons[i].addEventListener("click", increaseQty);
  }
  ...
  for (let i = 0; i < buttons.length; i++) {
    buttons[i].addEventListener("click", decreaseQty);
  }
  ...
  const delProduct= document.getElementsByClassName("fas fa-trash-alt");
  for (let i = 0; i < delProduct.length; i++) {
    delProduct[i].addEventListener("click", function () {
      deleteProduct(this);
    });
  }
